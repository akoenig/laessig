        ...
        create : {
            from : function (options) {
                ...
                fs.readdir(options.directory, function (error, files) {
                    ...
                    files = underscore.reject(files, function (filename) {
                        return path.extname(filename, options.extension) !== options.extension;
                    });
        ...
        has : function (snapshot) {
            return {
                file : function (filename) {
                    var file = underscore.filter(snapshot, function (entry) {
                ...
                newerThan : function (oldSnapshot) {
                    ...
                    if (snapshot.length < oldSnapshot.length) {
                        oldSnapshot.forEach(function (oldEntry) {
                            ...
                            if (!alive) {
                                ...
                                changedEntry = JSON.parse(JSON.stringify(oldEntry));
                    ...
                    // we have to deal with added files.
                        ...
                        snapshot.forEach(function (entry) {
                            ...
                            if (!found) {
                                // TODO: Use Object.create(entry)
                                changedEntry = JSON.parse(JSON.stringify(entry));
